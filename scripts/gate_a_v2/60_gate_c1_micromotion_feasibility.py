...
def b_exact(d,t):
 tau=float(t % d['T'])
 ev=prop(d['e1'],d['v1'],tau) if tau<=d['t1']+1e-14 else prop(d['e2'],d['v2'],tau-d['t1'])@d['u1'];modes=ev@d['V']*np.exp(1j*d['eps_full']*tau)[None,:];return (modes.conj().T@d['sm']@modes)[np.ix_(d['indices'],d['indices'])]
